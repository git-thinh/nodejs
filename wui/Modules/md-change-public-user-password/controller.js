function mdchangepublicuserpasswordCtrl_clear() {
...
function mdchangepublicuserpasswordCtrl($rootScope, $scope, $route, $controller, $http, $timeout, svCore, CONNECTION_ERROR) {
    var label = {};
    ...
    $scope.cancel = function () {
        $scope.user = userInit;
        mdchangepublicuserpasswordCtrl_clear();
    };
    ...
    $scope.submit = function () {
        ...
        $http.post(CONFIG.URL_API + 'public/validatePublicUser?socket-id=' + $rootScope.socketID, user).
            success(function (rsVali) {
                ...
                if (rsVali['result'] == 'success') {
                    ...
                    var con = label.Res_0703_01_04.split('{userName}').join(user.userName);
                    alertShow({
                        type: 'confirm',
                        title: label.Res_0703_01,
                        content: con,
                        cancel: function () {
                            $scope.user = userInit;
                            mdchangepublicuserpasswordCtrl_clear();
                        ...
                        ok: function () {
                            $http.post(CONFIG.URL_API + 'public/changePassword?socket-id=' + $rootScope.socketID, user).
